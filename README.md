...
В данный момент установлено `sequelize.sync({ force: true })`, что означает сброс всей текущей БД (все уже имеющиеся данные будут удалены) при каждом запуске сервера
